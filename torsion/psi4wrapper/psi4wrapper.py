...
def run_psi4(calculation, mol, method, basis, **psi4opts):
    ...
    stderr/stdout"""
    logging.debug(
        "Beginning Psi4 {} calculation with {}/{}".format(calculation, method, basis)
    )
    ...
    try:
        ...
        logging.error(
            "Failed to properly execute psi4 {} ({}/{})".format(
        ...
        logging.info(
            "Successful psi4 execution {} ({}/{})".format(calculation, method, basis)
        )
...
def psi4_calculation(
    ...
    **psi4opts
    ...
    if geom_opt_technique == "QM":
        logging.info(
            "Start geometry optimization on conformer %s angle %d", conf_name, angle_deg
        )
        ...
        calc_value, wavefcn = run_psi4(
            "optimize", psi_mol, opt_method, opt_basis, **psi4opts
        )
    ...
    if (
        ...
        spe_start = time.time()
        calc_value, wavefcn = run_psi4(
            "energy", psi_mol, spe_method, spe_basis, **psi4opts
        )
        if calc_value is None:
            logging.error(
    ...
    try:
        ...
        print(
            "Unable to save psi4 log data ", psi4_log_filename, " because of error: ", e
        )
