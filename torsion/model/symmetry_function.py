...
from torsion.utils import get_torsion_oeatom_list, get_torsion_oebond
from torsion.analysis import get_specific_dihedral_inchi_key
...
def GetThetaIJKLMatrix(mol, iAtoms, jAtom, kAtom, lAtoms, transform=True):
    ...
    for iAtom in iAtoms:
        for lAtom in lAtoms:
            tor_angle = OEGetTorsion(mol, iAtom, jAtom, kAtom, lAtom)
            if not transform:
...
class SymmetryFunction:
    def __init__(self):
        ...
        self.elemList = [OEElemNo_H, OEElemNo_C, OEElemNo_N, OEElemNo_O,
                         OEElemNo_F, OEElemNo_S, OEElemNo_Cl, "pc", "nc"]
    ...
    def GetTorsionEnvAtoms(self, elem, bgnAtom, endAtom, envMol):
        elemEnvList = []
        for envAtom in OEGetSubtree(bgnAtom, endAtom):
            if elem == 'pc' and envAtom.GetFormalCharge() >= 1:
                elemEnvList.append(envAtom)
            elif elem == 'nc' and envAtom.GetFormalCharge() <= -1:
                elemEnvList.append(envAtom)
                ...
                if len(lcoords) == 0:
                    for ita in self.itaVec:
                        for rc in self.rcTorVec:
                            tsf.append(0.0)
                    continue
                klX, klY, klZ, rkl, rkl2 = GetPairwiseDistanceMatrix([kcoords], lcoords)
                ilX, ilY, ilZ, ril, ril2 = GetPairwiseDistanceMatrix(icoords, lcoords)
    ...
    def GetTorsionCenterAsOEMol(self, mol):
        ...
        refMol = OEMol()
        refAtom = refMol.NewAtom(OEElemNo_C)
    ...
    def CalculateSymmetryFunction(self, envMol):
        ...
        refAtom = refMol.GetAtom(OEHasAtomIdx(0))
                ...
                if len(kcoords) == 0:
                    for ita in self.itaVec:
                        for rc in self.rcAngVec:
                            asf.append(0.0)  # angular
                    continue
                ikX, ikY, ikZ, rik, rik2 = GetPairwiseDistanceMatrix([icoords], kcoords)
                jkX, jkY, jkZ, rjk, rjk2 = GetPairwiseDistanceMatrix(jcoords, kcoords)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate symmetry function representing '
                                                 'the chemical environment around the specified torsion.')
    parser.add_argument('--input', type=str, help='Input file with molecules containing TORSION_ATOMS_FRAGMENT '
                                                'sd property, specifying 1-based torsion atom indices.')
    ...
    # input file
    logging.debug("Opening input file %s", args.input)
    ifs = oechem.oemolistream()
    if not ifs.open(args.input):
        oechem.OEThrow.Fatal("Unable to open input file %s" % args.input)

    output_list = []
    for mol in ifs.GetOEMols():
        inchi_key = get_specific_dihedral_inchi_key(mol)
        sf_elements = get_sf_elements(mol)

        output = [inchi_key]
        output.extend(sf_elements)
        output_list.append(output)

    ifs.close()

    columns = ['dihedral_inchi']
    sf_headers = ['sf_%d'%(i+1) for i in range(len(output_list[0]) - 1)]
    columns.extend(sf_headers)
