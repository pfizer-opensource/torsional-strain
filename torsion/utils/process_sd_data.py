...
def generate_energy_profile_sddata(energy_profile):
    ...
    energy_profile[energy_profile[:,0] > 180, 0] = energy_profile[energy_profile[:,0] > 180, 0] - 360 
    ...
    sddata = 'Torsion scan profile (data=[' + \
...
def is_amide_torsion(mol):
    ...
    try:
        if torsion_atoms[1].IsCarbon() and oechem.OEHasDoubleBondO(torsion_atoms[1])\
                and torsion_atoms[2].IsNitrogen() and torsion_atoms[2].GetExplicitHCount() > 0:
            return True
        if torsion_atoms[1].IsNitrogen() and torsion_atoms[1].GetExplicitHCount() > 0 \
                and torsion_atoms[2].IsCarbon() and oechem.OEHasDoubleBondO(torsion_atoms[2]):
            return True
...
def has_protonated_double_bonded_ring_nitrogen(mol):
    for atom in mol.GetAtoms(oechem.OEAndAtom(oechem.OEAndAtom(oechem.OEIsNitrogen(),
...
def generate_energy_profile_sd_data_1d(data):
    ...
    # Generate sddata string
    sddata = 'Torsion scan profile (data=[' + \
             ','.join(["{:.2f}@{:.2f}".format(energy, angle)
                       for angle, energy in zip(angles, rel_energies)]) + \
             '], '+ \
             'xlab=[Torsion angle (deg)], xScale=[-180.0, 180.0], ' + \
             'ylab=[Energy (kcal/mol)], yScale=[0,{:.2f}], type=[spline])'.format(max(rel_energies))
...
def extract_energy_profile_sd_data1d(sddata):
    ...
    """
    energies = np.array([float(x[0]) for x in [row.split('@') for row in sddata.split('[')[1].split(']')[0].split(',')]])
    angles = np.array([float(x[1]) for x in [row.split('@') for row in sddata.split('[')[1].split(']')[0].split(',')]])
